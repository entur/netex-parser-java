...
class TestUpdateStopPlaces {
  ...
  void testStopPlacesCanBeUpdatedAndRemoved() throws IOException {
    Assertions.assertEquals(
      1,
      index.getStopPlaceIndex().getAllVersions("NSR:StopPlace:9999").size()
    );
    NetexParser parser = new NetexParser();
    File file = new File("src/test/resources/StopPlaceUpdated.xml");
    NetexEntitiesIndex newIndex = parser.parse(
      Files.newInputStream(file.toPath())
    );
    Collection<StopPlace> updatedStopPlace = newIndex
      .getStopPlaceIndex()
      .getAllVersions("NSR:StopPlace:9999");
