...
class TestStopPlacesExport {
  ...
  static void init() {
    try {
      ...
      index =
        parser.parse("src/test/resources/CurrentwithServiceFrame_latest.zip");
    } catch (Exception e) {
  ...
  void testPublicationTimestamp() {
    Assertions.assertEquals(
      LocalDateTime.parse("2021-05-04T03:24:52.46"),
      index.getPublicationTimestamp()
    );
  }
  ...
  void testGetTopographicPlace() {
    TopographicPlace topographicPlace = index
    ...
    Assertions.assertEquals(
      "no",
      topographicPlace.getCountryRef().getRef().value()
    );
  }
  ...
  void testGetPassengerStopAssignment() {
    ScheduledStopPoint scheduledStopPoint = index
      .getScheduledStopPointIndex()
      .getLatestVersion("NSR:ScheduledStopPoint:S5");
    Collection<PassengerStopAssignment> passengerStopAssignments = index
      .getPassengerStopAssignmentsByStopPointRefIndex()
      .get(scheduledStopPoint.getId());
  ...
  void testGetParkingsByParentSiteRef() {
    Collection<Parking> parkings = index
      .getParkingsByParentSiteRefIndex()
      .get("NSR:StopPlace:337");
