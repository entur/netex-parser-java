...
class VehicleScheduleFrameTest {
  ...
  void testJourneys() {
    ...
    for (Block block : blocks) {
      ...
      if (
        block.getJourneys() != null &&
        block
          .getJourneys()
          .getJourneyRefOrJourneyDesignatorOrServiceDesignator() !=
        null
      ) {
        for (JAXBElement<?> journeyElement : block
          .getJourneys()
          .getJourneyRefOrJourneyDesignatorOrServiceDesignator()) {
  ...
  void testDayTypes() {
    ...
    for (Block block : blocks) {
      ...
      if (
        block.getDayTypes() != null &&
        block.getDayTypes().getDayTypeRef() != null
      ) {
        for (JAXBElement<? extends DayTypeRefStructure> dayTypeRef : block
          .getDayTypes()
          .getDayTypeRef()) {
  ...
  void testGetJourneys() {
    ...
    List<JAXBElement<?>> journeys =
      result.getJourneyRefOrJourneyDesignatorOrServiceDesignator();
    Assertions.assertEquals(2, journeys.size());
  }

  @Test
  void testVehicleScheduleFrames() {
    Collection<VehicleScheduleFrame> vehicleScheduleFrame =
      index.getVehicleScheduleFrames();
