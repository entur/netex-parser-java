...
class TimeTableFrameParser
  ...
  private final List<DatedServiceJourney> datedServiceJourneys =
    new ArrayList<>();
  private final Multimap<String, DatedServiceJourney> datedServiceJourneyByServiceJourneyId =
    ArrayListMultimap.create();

  private final List<DeadRun> deadRuns = new ArrayList<>();

  private final List<ServiceJourneyInterchange> serviceJourneyInterchanges =
    new ArrayList<>();
  private final Multimap<String, ServiceJourneyInterchange> serviceJourneyInterchangesByServiceJourneyId =
    ArrayListMultimap.create();
  ...
  void parse(Timetable_VersionFrameStructure frame) {
    ...
    informOnElementIntentionallySkipped(
      LOG,
      frame.getAccessibilityAssessment()
    );
    ...
    informOnElementIntentionallySkipped(
      LOG,
      frame.getFlexibleServiceProperties()
    );
    ...
    informOnElementIntentionallySkipped(
      LOG,
      frame.getServiceCalendarFrameRef()
    );
  ...
  void setResultOnIndex(NetexEntitiesIndex netexIndex) {
    ...
    netexIndex
      ...
      .putAll(datedServiceJourneyByServiceJourneyId);

    netexIndex.getDeadRunIndex().putAll(deadRuns);

    netexIndex
      .getServiceJourneyInterchangeIndex()
      .putAll(serviceJourneyInterchanges);
    netexIndex
      .getServiceJourneyInterchangeByServiceJourneyRefIndex()
      .putAll(serviceJourneyInterchangesByServiceJourneyId);
  ...
  private void parseInterchanges(
    JourneyInterchangesInFrame_RelStructure journeyInterchangesElement
  ) {
    ...
    for (Interchange_VersionStructure it : journeyInterchangesElement.getServiceJourneyPatternInterchangeOrServiceJourneyInterchange()) {
      if (it instanceof ServiceJourneyInterchange serviceJourneyInterchange) {
        ...
        serviceJourneyInterchangesByServiceJourneyId.put(
          fromRef,
