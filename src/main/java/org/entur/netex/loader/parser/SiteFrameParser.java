...
class SiteFrameParser extends NetexParser<Site_VersionFrameStructure> {
  ...
  private final Collection<FlexibleStopPlace> flexibleStopPlaces =
    new ArrayList<>();

  private final Collection<GroupOfStopPlaces> groupsOfStopPlaces =
    new ArrayList<>();
  ...
  private final Collection<GroupOfTariffZones> groupsOfTariffZones =
    new ArrayList<>();

  private final Collection<TopographicPlace> topographicPlaces =
    new ArrayList<>();
  ...
  public void parse(Site_VersionFrameStructure frame) {
    ...
    if (frame.getGroupsOfStopPlaces() != null) {
      parseGroupsOfStopPlaces(
        frame.getGroupsOfStopPlaces().getGroupOfStopPlaces()
      );
    }

    if (frame.getFlexibleStopPlaces() != null) {
      parseFlexibleStopPlaces(
        frame.getFlexibleStopPlaces().getFlexibleStopPlace()
      );
    }
    ...
    if (frame.getTopographicPlaces() != null) {
      parseTopographicPlaces(
        frame.getTopographicPlaces().getTopographicPlace()
      );
    }
    ...
    if (frame.getGroupsOfTariffZones() != null) {
      parseGroupsOfTariffZones(
        frame.getGroupsOfTariffZones().getGroupOfTariffZones()
      );
    }
    ...
    informOnElementIntentionallySkipped(
      LOG,
      frame.getCheckConstraintThroughputs()
    );
    ...
    informOnElementIntentionallySkipped(
      LOG,
      frame.getPointOfInterestClassifications()
    );
    informOnElementIntentionallySkipped(
      LOG,
      frame.getPointOfInterestClassificationHierarchies()
    );
  ...
  private void parseFlexibleStopPlaces(
    Collection<FlexibleStopPlace> flexibleStopPlacesList
  ) {
    flexibleStopPlaces.addAll(flexibleStopPlacesList);
  }

  private void parseGroupsOfStopPlaces(
    Collection<GroupOfStopPlaces> groupsOfStopPlacesList
  ) {
  ...
  private void parseTariffZones(
    List<JAXBElement<? extends Zone_VersionStructure>> tariffZoneList
  ) {
  ...
  private void parseTopographicPlaces(
    Collection<TopographicPlace> topographicPlaceList
  ) {
   ...
   * @param stopPlaceId
   */
  private void parseQuays(
    Quays_RelStructure quayRefOrQuay,
    String stopPlaceId
  ) {
    ...
    for (JAXBElement<?> jaxbQuay : quayRefOrQuay.getQuayRefOrQuay()) {
      if (jaxbQuay.getValue() instanceof Quay quay) {
        ...
        if (!stopPlaceIdByQuayId.containsKey(quayId)) {
          ...
          Quay latestVersion = NetexVersionHelper.latestVersionedElementIn(
            quays.get(quayId)
          );
  ...
  private void parseGroupsOfTariffZones(
    List<GroupOfTariffZones> groupOfTariffZones
  ) {
